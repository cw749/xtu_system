    ...
    <div class="teacher-page">
        <section class="page-card toolbar-card">
            <div>
                ...
                <p class="page-subtitle">维护教师档案，同时支持批量删除、批量导入和 CSV 导出。</p>
            </div>

            <el-form :inline="true" :model="query" class="query-form">
                <el-form-item>
                    <el-input v-model="query.keyword" clearable placeholder="工号或姓名" />
                </el-form-item>
                <el-form-item>
                ...
                <el-form-item>
                    ...
                    <el-button v-if="hasPermission('personnel:teacher:create')" type="success" @click="openCreateDialog">新增教师</el-button>
                    <el-button v-if="hasPermission('personnel:teacher:create')" type="warning" @click="openImportDialog">批量导入</el-button>
                    <el-button v-if="hasPermission('personnel:teacher:view')" type="info" @click="handleExport">导出数据</el-button>
                    <el-button
                        v-if="hasPermission('personnel:teacher:delete')"
                        type="danger"
                        plain
                        :disabled="!selectedTeacherIds.length"
        ...
        <section class="page-card table-card">
            <el-table ref="tableRef" :data="teacherList" stripe @selection-change="handleSelectionChange">
                ...
                <el-table-column prop="titleName" label="职称" min-width="160" show-overflow-tooltip />
                ...
                <el-table-column label="操作" width="260" fixed="right">
                    <template #default="{ row }">
                        <el-button v-if="hasPermission('personnel:teacher:update')" link type="primary" @click="openEditDialog(row.id)">编辑</el-button>
                        <el-button v-if="hasPermission('personnel:teacher:delete')" link type="danger" @click="handleDelete(row.id)">删除</el-button>
                        <el-button
                            v-if="!row.userId && hasPermission('personnel:teacher:account')"
                            link
                            type="success"
        ...
        <el-dialog v-model="dialogVisible" :title="dialogTitle" width="660px" destroy-on-close>
            ...
            <template #footer>
                ...
                <el-button
                    v-if="hasPermission(form.id ? 'personnel:teacher:update' : 'personnel:teacher:create')"
                    type="primary"
                    :loading="submitting"
                    @click="handleSubmit"
                >
        ...
        <el-dialog v-model="accountDialogVisible" :title="accountDialogTitle" width="520px" destroy-on-close>
            ...
            <template #footer>
                ...
                <el-button
                    v-if="hasPermission('personnel:teacher:account')"
                    type="primary"
        ...
        <el-dialog v-model="importDialogVisible" title="批量导入教师" width="760px" destroy-on-close>
            <el-alert
                type="info"
                show-icon
                :closable="false"
                title="每行一条记录，支持英文逗号、中文逗号或 Tab 分隔。字段顺序：工号, 姓名, 性别, 部门ID/部门名称, 职称, 手机号, 邮箱, 状态, 备注。"
            />
            ...
            <el-input
                ...
                :rows="10"
                resize="none"
                placeholder="例如：T2026001,王老师,女,信息工程学院,讲师,13800138010,teacher@example.com,在职,新入职"
            />
...
const dialogTitle = computed(() => (form.id ? '编辑教师' : '新增教师'))
...
const importExample = computed(() => [
    '工号,姓名,性别,部门,职称,手机号,邮箱,状态,备注',
    'T2026001,王老师,女,信息工程学院,讲师,13800138010,teacher@example.com,在职,新入职',
    'T2026002,李老师,男,2002,副教授,13800138011,li@example.com,在职,骨干教师'
].join('\n'))
...
async function loadTeachers() {
    const response = await getTeacherPage(query)
...
function openAccountDialog(row) {
    ...
    accountTargetName.value = row.teacherName
    resetAccountForm()
    accountForm.username = row.teacherNo
...
async function handleCreateAccount() {
    ...
    try {
        await createTeacherAccount(accountTargetId.value, accountForm)
...
async function handleRemoveAccount(row) {
    ...
    await removeTeacherAccount(row.id)
    ElMessage.success('教师账号已注销')
    loadTeachers()
}

async function handleDelete(id) {
    await ElMessageBox.confirm('删除教师后若已绑定账号，会同步注销该账号，是否继续？', '确认删除', {
        type: 'warning'
    })
...
async function handleBatchDelete() {
    await ElMessageBox.confirm(`将批量删除 ${selectedTeacherIds.value.length} 条教师记录，是否继续？`, '确认批量删除', {
        type: 'warning'
    })
    await batchDeleteTeachers(selectedTeacherIds.value)
...
async function handleImport() {
    let payload
    try {
        payload = parseTeacherImportText()
    } catch (error) {
        ElMessage.error(error.message || '教师导入失败')
        return
    }

    importing.value = true
    try {
        const response = await importTeachers(payload)
        ElMessage.success(`成功导入 ${response.data} 条教师记录`)
...
async function handleExport() {
    const blob = await exportTeachers(buildExportQuery())
...
function parseTeacherImportText() {
    ...
    return lines
        .filter((line, index) => {
            ...
            return !(index === 0 && firstColumn === '工号')
        })
        .map((line, index) => {
            const columns = splitImportLine(line)
            if (columns.length < 4) {
                throw new Error(`第 ${index + 1} 行至少需要填写工号、姓名、性别和部门`)
            }
            ...
            return {
                ...
                phone,
                email,
                status: parseTeacherStatus(statusText),
                remark
            }
        })
}
